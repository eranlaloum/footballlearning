...
q_table = []
with open('Q_table26.csv', 'r') as csvfile:
...
exploration_rate=0.010000499803739864 #epsilon
...
for episode in range(num_episodes):
    ...
    if exploration_rate_threshhold>exploration_rate:
        ...
        if (action==0 and first_robot_position==0) or (action==1 and first_robot_position==2): # if the robot perform illegal movement
            wrong_movement=True
    ...
    print(f"hit reward: {reward_each_step}")
    q_table[old_state][action] = q_table[old_state][action] * (1 - learinng_rate) + learinng_rate * (reward_each_step + discount_rate * np.max(q_table[new_state]))
    reward_each_step = 0
    old_state = new_state
    #exploration rate decay
    exploration_rate = min_exploration_rate + (max_exploration_rate - min_exploration_rate) * np.exp(-exploration_decay_rate*(episode+14500))
...
################ Writer #################
with open('Q_table27.csv', 'a', newline='') as file:
